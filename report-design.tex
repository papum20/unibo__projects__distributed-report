...
\begin{itemize}
  ...
  \item messages for system commands (over REST API), from the user to the web
  viewer (which will then communicate them to the correct recipient);
  \item messages for system commands (\textit{sysctrl} MQTT messages), from the
  web viewer (after receiving one from the viewer), either towards individual
  vehicles or towards all at the same time in broadcast;
  \item messages for detected collisions, from the simulation component, to be
  displayed to the viewer;
  \item a message sent by a vehicle to communicate that it has reset it's
  position (about reset in \ref{arch:behaviour_simulation}), for displaying
  purposes;
\end{itemize}
...
enough margin. Hence, if all inside vehicles have been stationary for a
certain amount of time (e.g. 2 seconds), the controller will trigger a command 
to accelerate to all; if there are some that have more margin
...
alternating an acceleration and an emergency brake soon after. In an ideal
world, all vehicles in a line could start accelerating at the same time and same
pace, without colliding, but they would still have to go extremely slow, in
order to be ready for any emergency brake with that small margin in front.
...
overflooding of messages, there's a cooldown (e.g. 1 second) to identify again
collisions between the same pair of vehicles.
...
the latest state. Therefore, data freshness is prioritized over consistency and
purging old data is better than synchronizing it.
...
necessary for emergency) when inside the roundabout should reassure (but not
guarantee), analogously to what discussed about deadlocks in the controller's
section (\ref{arch:behaviour_controller}).
\\
\\
...
reduced load; with a message topic dedicated to each zone, a vehicle would then
