...
needed, a test was developed and only then that would have been implemented with
a separate function (so that it could be tested alone). Unfortunately, this
didn't last long, as the required features, and the related functions, soon
became too complex. In the first place, this means that a functions becomes too
hard to test, with too many possible normal or edge cases, and unexpected ones.
The main problem, however, is that it was hard to even know in advance how the
controller's or vehicle's logic should have come out at the end, so developing a
test for a function that would have been changed or even removed soon after
would have taken too much time away from the distributed systems related topics.
...
For communication and interaction testing, manual tests seemed enough
(\ref{validation-acceptance}).
\\
\\
\textit{pytest} (9.1.1) has been used for unit testing. All tests (in the
$src/tests/$ folder) can be run, from the project's root directory, with:
\[
  pytest
\]

One should first install the packages required by the tested services (all
...
\\
\\
The developed unit tests for physics ($test_physics.py$) and mathematics
($test_math_utils.py$ and $test_roundabout.py$) regard kinematics and
calculations: the test, with some cases, that distances are calculated
...
Unit tests for the the controller ($test_controller.py$) test the command
...
Finally, unit tests for the vehicle ($test_vehicle.py$) perform analogous tests
...
Being the subject a simulated real scenario, verifying that everything goes as
expected by looking at the visualization is straight-forward, although debugging
requires much more effort, buy deeply analyzing the code and correlating it with
what happens in the simulation, or by checking logs and vehicles and controller
statistics. The behaviour one should expect to see is what described in the
requirements (\ref{req:requirements}).
...
Once run the simulation, tests were made with varying amounts of vehicle into
failsafe of disconnected vehicles.
\\
\\
When the project can be considered working, some possible evaluation metrics at
disposal, available on the simulation page (refer to the user guide,
\ref{user-guide}), are the number of collisions happened and the total number of
spawned vehicles in a certain amount of time, to measure the total throughput of
the roundabout. Here follow some example runs; refer to the self-evaluation
(\ref{self-evaluation}) for a short analysis.
