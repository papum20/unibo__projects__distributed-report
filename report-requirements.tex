...
In general terms, the functional requirements of the project concern the
building of a simulated environment, together with an application to visualize
it and some commands to manage it.
...
\begin{itemize}
  ...
  number, e.g. 30).
  \item 2.1 --- From a top-down perspective, a car's surface covers a rectangle,
  both visually and physically (e.g. for collisions).
  \item 2.2\label{req:req_22} --- A car is able to move in space, at a speed
  which can be tuned between certain minimum and maximum values by changing its
  acceleration. The physical model thus comprehends simple uniform motion and
  uniformly accelerated motion, both linear (for a straight road) and circular
  (around the roundabout). Acceleration also ranges from a negative minimum (for
  braking) to a positive maximum; the minimum amount (i.e. strongest braking)
  should be used as little as possible, to reduce the car's wear.
  ...
  acceleration.
  \item 4 --- The system shall run a simulation, where vehicles move from each
  one's start to end point while communicating with the controller.
  ...
  failsafe (correctly communicating towards the controller, but not receiving, or
  ...
  \item 5 --- The system shall offer ways to both visualize and give some
  commands to the simulation.
  \item 5.1 --- The system allows to visualize the simulation in real time,
  together with additional data on the vehicles, such as current speed and
  acceleration.
  \item 5.2 --- The system allows to send commands to control the simulation
  including: pausing and resuming the simulation, changing the state of one or
  more vehicles.
\end{itemize}
...
\\
\\
Non-functional requirements are instead related to distributed systems matters:
...
\begin{itemize}
  ...
  \textbf{interoperability}: the system should aim to be open and interoperable
  with different implementations (for example, from different car brands, using
...
course's project requirements and which I personally have little or no
experience with:
\begin{itemize}
...
requirements themselves and one component needs to know about others' failures.
...
should be granted at all times (requirement 6.1, \ref{req:req_fault}). Even with
network issues, the system should provide \textbf{reliability} globally.
...
expected to grow a lot. For a different, larger scenario, for example trying to
coordinate traffic over a wider area, this may become a more urgent issue.
...
As stated in constraint 6.2 (\ref{req:req_openness}), the system should be open
to potentially welcome external vehicles' implementations, for example even with
different languages and technologies.

\subsubsection{Evolvability, maintainability}\label{req:distr_evolv}
The system doesn't require frequent updates, unless there are changes to the
...
the controller to each one: a significant distance can be ridden in less than a
second.

Regarding computational performance and network load, because the system isn't
expected to grow significantly (as mentioned in \ref{req:distr_scalability}),
there shouldn't be a need for particularly powerful expensive servers and
network interfaces to sustain enough vehicles.
...
don't seem to be any measures which could be employed in the development to
heavily impact the budget (also refer to scalability,
\ref{req:distr_scalability}, for the expected system's dimensions).
