...
$http://localhost:8080$. Cars should be seen moving, proceeding proceed
endlessly.
\\
\\
A \textbf{dashboard} on the left of the page displays:
\begin{itemize}
  \item in the first row, in order, the total number of spawned vehicle in the
  ...
  for exiting); start and destination road indexes (start number $->$ end
  ...
  ($hours:minutes:seconds$), conflicting vehicles ($v_1-v_2$), conflicting
  vehicles' navigation statuses.
\end{itemize}
...
The bash script $src/ctrl.sh$ can be used directly. It expects one command,
optionally followed by the vehicles to address:
\begin{itemize}
  \item without specified vehicles, the command is addressed to everyone (e.g.
useful for pausing or resuming);
  \item if it's a number $N$ of one or two digits, $N$ random vehicles will be
addressed;
  \item otherwise, a string (different from the previously described $N$) will
  be used as prefix of a vehicle id and the single vehicle will be addressed, if
  found and unique.
\end{itemize}

Available can be written as as entire words or by the short letter, uppercase or
lowercase:
\begin{itemize}
  \item $pause$ ($p$)
  \item $resume$ ($r$)
  \item $failsafe$ ($f$): forcefully set to failsafe
  \item $controller$ ($c$): undo failsafe (set to normal)
  \item $disconnect$ ($d$): forcefully set to disconnected
  \item $reconnect$ ($n$): undo disconnect (set to normal, like for $c$)
\end{itemize}
...
\begin{itemize}
    ...
    EXIT\_FAILSAFE, ENTER\_DISCONNECTED, EXIT\_DISCONNECTED}). Optionally accepts
    \texttt{vehicle\_id} to target a specific node, or \texttt{vehicle\_count}
    to apply the fault to a random subset of nodes, otherwise will target all if
    left empty.
\end{itemize}
